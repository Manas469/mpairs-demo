...
st.title("�m-PAIRS | Enterprise Gateway")
#� Access Control & Inputs
api_key = st.text_input("Access Token", type="password")
query_input = st.text_area( "Grievance / Audit Target Text", placeholder="Type or paste your query/context parameters here...", height=150
)
# � NEW: Document Ingestion Field (Supports PDF, Word, and Plain Text)
uploaded_file = st.file_uploader( "Upload Supporting Evidence / Reference Files", type=["pdf", "docx", "txt"]
)
...
MPAIRS_SECURE_ENDPOINT = "https://engine.totalstrategies.workers.dev/v1/execute" headers = {"Authorization": f"Bearer {api_key}"}
...
response = requests.post(
MPAIRS_SECURE_ENDPOINT,
json=data_payload, headers=headers, timeout=60
)
